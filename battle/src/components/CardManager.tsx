...
interface CardManagerProps {
  ...
  discardPile: Card[];
  exhaustPile: Card[];
  availableCards: Card[]; // 从API获取的可用卡牌
  isLoading?: boolean;
...
export function CardManager({
  ...
  const getPileByName = (pileName: CardPile): Card[] => {
    switch (pileName) {
      case 'deck': return deck;
      case 'hand': return hand;
      case 'drawPile': return drawPile;
  ...
  const getSetterByName = (pileName: CardPile): ((cards: Card[]) => void) => {
    switch (pileName) {
      case 'deck': return onSetDeck;
      case 'hand': return onSetHand;
      case 'drawPile': return onSetDrawPile;
  ...
  const moveCard = (from: CardPile, to: CardPile, cardId: string) => {
    const sourcePile = getPileByName(from);
    const targetPile = getPileByName(to);

    const card = sourcePile.find(c => c.id === cardId);
    if (!card) return;

    // 从源移除
    const sourceSetter = getSetterByName(from);
    sourceSetter(sourcePile.filter(c => c.id !== cardId));

    // 添加到目标
    const targetSetter = getSetterByName(to);
          ...
          <div className="flex gap-2">
            <Dialog>
              <DialogTrigger asChild>
                <Button size="sm" variant="outline" disabled={isLoading || availableCards.length === 0}>
