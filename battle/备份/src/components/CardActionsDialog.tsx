...
import { Search, Layers, Package, Flame, Trash2 } from 'lucide-react';
import { cn } from '@/lib/utils';

type CardDestination = 'draw' | 'discard' | 'exhaust';
...
interface PileManagerDialogProps {
  ...
  onMoveCardTo: (cardId: string, destination: 'draw' | 'discard' | 'exhaust') => void;
}
...
export function PileManagerDialog({
  ...
  const handleMoveCard = (cardId: string, destination: 'draw' | 'discard' | 'exhaust') => {
    // 如果目标就是当前堆，不执行操作
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                {getDestinationOptions().map(option => (
                  <Button 
                    ...
                    variant="outline" 
                    onClick={() => {
                      handleMoveCard(selectedCard.id, option.value as 'draw' | 'discard' | 'exhaust');
